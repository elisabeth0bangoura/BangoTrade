...
export default function PortfolioValueDecreasedSheet () {
  ...
  const { t } = useTranslation();
    ...
    const PortfolioValueDecreased_Sheet = useRef(null);
...
const formatMoneyMyInvestmnt = useCallback((price) => {
   ...
   const MyInvestmentsrenderItem = ({ item }) => {
    ...
    return (
      ...
      return (
        ...
        <ActionSheet 
        ref={PortfolioValueDecreased_Sheet}
        gestureEnabled={true}
        isModal={false}
  ...
  The value of your portfolio and investments is calculated in real-time based on available prices.
  Temporary fluctuations in value may occur, especially after events such as stock splits or mergers. 
  As a result, you may see sharp increases or decreases in the chart.
...
Temporary changes in value may occur in the following cases:
</Text>
...
• A company has undertaken corporate  {`\n`}  actions such as stock splits or mergers. {`\n`} {`\n`}
• Trading in an instrument has been halted. {`\n`} {`\n`}
• Stock market data is being transmitted with {`\n`} a delay. {`\n`}
...
Temporary fluctuations in value are typically resolved within 2-3 business days. 
If a position's value hasn't been adjusted after 3 business days, 
please reach out to us by selecting the relevant investment under Current Investments.
...
<Text style={{
  ...
  If you need help with an investment, please go back to the 
   previous page and select an investment to start a chat.
  </Text>
...
<TouchableOpacity onPress={() => {
          ...
          posthog.capture('close_portfolio_value_decreased_Sheet', {
            screen: 'PortfolioValueDecreased_Sheet',
            $screen_name: 'PortfolioValueDecreased_Sheet',
            timestamp: new Date().toISOString(),
            });
  
      
      SheetManager.hide("PortfolioValueDecreased_Sheet"); // Now hide the sheet after a delay
