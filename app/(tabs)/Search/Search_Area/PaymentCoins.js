...
export default function PaymentCoins({ SearchIndex }) {
  ...
  // ✅ Fetch Payment Coins Data (Without Charts)
  const fetchCoinsData = async () => {
    ...
    try {
      console.log("🔥 Fetching Payment Coins...");
      ...
      if (!data?.length) {
        console.warn("❌ No Payment Coins found.");
        setCoinsData([]);
        setIsDataFetched(false);
        return;
      }
      ...
      const filteredCoins = data
        .filter((coin) =>
          ["monero", "xmr", "dash", "zec", "bitcoin-cash", "litecoin", "xrp", "nano", "stellar", "dogecoin"].includes(
            coin.id.toLowerCase()
          ) ||
          ["xmr", "dash", "zec", "ltc", "bch", "xrp", "xlm", "nano", "doge"].includes(
    ...
    //  console.log("✅ Found Payment Coins:", filteredCoins);
      ...
      setCoinsData(
        filteredCoins.map((coin) => ({
          ...
          CoinChartColor: coin.price_change_percentage_24h > 0 ? "#00CE39" : "#FF1B1E", // ✅ Green/Red Chart Color
        }))
      );
