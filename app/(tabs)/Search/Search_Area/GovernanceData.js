...
export default function GovernanceData({ SearchIndex }) {
  ...
  const [governanceCoins, setGovernanceCoins] = useState([]);
  const [isFetching, setIsFetching] = useState(false);
  const [isDataFetched, setIsDataFetched] = useState(false);

  const loadedCharts = useRef(new Map()); // ✅ Persistent cache for chart data
  const governanceCoinsMemoized = useMemo(() => governanceCoins, [governanceCoins]);

  // Governance token list (excluding AAVE, UNI, SUSHI)
  const governanceTokenIds = ["compound", "maker", "aragon", "optimism"];
  ...
  // ✅ Fetch Governance Coins Data (Without Charts)
  const fetchGovernanceTokens = async () => {
    ...
    try {
   //   console.log("🔥 Fetching governance tokens...");
      const res = await fetch(
        `https://pro-api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${governanceTokenIds.join(",")}`,
        options
      );
      const data = await res.json();

      if (!data?.length) {
        console.warn("❌ No governance tokens found.");
        setGovernanceCoins([]);
        setIsDataFetched(false);
        return;
      }

    //  console.log("✅ Found Governance Tokens:", data);
