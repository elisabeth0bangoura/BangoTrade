...
export default function SearchFilter_SmartContract({ SearchIndex3 }) {
  ...
  // ✅ Memoize data
  const smartContractCoinsMemoized = useMemo(() => [...smartContractCoins], [smartContractCoins]);
  ...
  const fetchSmartContractCoins = useCallback(
    debounce(async (sortOrder, sortBy) => {
      if (isFetching) return;
      setIsFetching(true);
  
      try {
        console.log(`🔥 Fetching Smart Contract Coins... Sorting: ${sortBy} (${sortOrder})`);
  
        // ✅ Step 1: Fetch top 100 coins (Ordered by Market Cap)
        const response = await fetch(
          "https://pro-api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1&price_change_percentage=1h,24h,7d",
          options
        );
        const data = await response.json();
  
        if (!Array.isArray(data) || data.length === 0) {
          console.warn("❌ No Smart Contract Coins found.");
          setSmartContractCoins([]);
          setIsDataFetched(false);
          return;
        }
  
        console.log("📊 Sample API Response (First 5 Coins):", data.slice(0, 5)); // ✅ Debugging Log
  
        // ✅ Step 2: Define a whitelist of smart contract platforms
        const knownSmartContractPlatforms = [
          "ethereum",
          "solana",
          "polygon",
          "binancecoin",
        ...
        // ✅ Step 3: Filter Smart Contract Coins (Strict)
        let sortedData = data
          .filter((coin) =>
            knownSmartContractPlatforms.includes(coin.id) // ✅ Only include known smart contract platforms
          )
          .map((coin) => ({
            id: coin.id,
            name: coin.name,
        ...
        console.log("✅ Filtered Smart Contract Coins (First 5):", sortedData.slice(0, 5)); // ✅ Debugging Log
  
        // ✅ Sorting Logic (Price, 1h, 24h, 7d, Volume, Market Cap)
        switch (sortBy) {
          case "price":
          ...
          case "1h":
            sortedData.sort((a, b) =>
              sortOrder === "percent_change_1h_desc"
                ? b.price_change_percentage_1h - a.price_change_percentage_1h
                : a.price_change_percentage_1h - b.price_change_percentage_1h
            );
            break;
          case "24h":
            sortedData.sort((a, b) =>
          ...
          case "7d":
            sortedData.sort((a, b) =>
              sortOrder === "percent_change_7d_desc"
                ? b.price_change_percentage_7d - a.price_change_percentage_7d
                : a.price_change_percentage_7d - b.price_change_percentage_7d
            );
            break;
          case "volume":
        ...
        console.table(sortedData.slice(0, 5)); // ✅ Debugging Log
  
        setSmartContractCoins([...sortedData]);
        setIsDataFetched(true);
      } catch (error) {
        console.error("🚨 Error fetching Smart Contract Coins:", error);
        setIsDataFetched(false);
      } finally {
        setIsFetching(false);
      }
  ...
  useEffect(() => {
    ...
    if (SearchIndex3 === 0) {
      fetchSmartContractCoins();
    } else if (SearchIndex3 === -1) {
      setSmartContractCoins([]);
      setIsDataFetched(false);
    }
  }, [SearchIndex3, fetchSmartContractCoins]);
...
<ScrollView horizontal style={{
 ...
 <TouchableOpacity onPress={() => {
  ...
  fetchSmartContractCoins(newSortOrder, "price");
  ...
  fetchSmartContractCoins(newSortOrder, "1h");
        }}
        style={{
             backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
             paddingVertical:  height(1) ,
      ...
      fetchSmartContractCoins(newSortOrder, "24h");
        }}
        style={{
             backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
             paddingVertical:  height(1) ,
  ...
  fetchSmartContractCoins(newSortOrder, "7d");
        }}
        style={{
             backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
             paddingVertical:  height(1) ,
        ...
        onPress={() => {
          ...
          fetchSmartContractCoins(newSortOrder, "volume");
      }}
        style={{
         backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
         paddingVertical:  height(1) ,
        ...
        <TouchableOpacity 
          onPress={() => {
            ...
            fetchSmartContractCoins(newSortOrder, "market_cap");
        }}
        style={{
         backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
         paddingVertical:  height(1) ,
  ...
  {isDataFetched ? (
        <FlashList
          data={smartContractCoinsMemoized}
