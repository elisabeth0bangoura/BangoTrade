...
export default function SearchFilter_Losers({ SearchIndex3 }) {
  ...
  const trendingCoinsMemoized = useMemo(() => [...trendingCoinsData], [trendingCoinsData]);
    ...
    const fetchLosersCoins = useCallback(
      debounce(async (sortOrder, sortBy) => {
        if (isFetching) return;
        setIsFetching(true);
    
        try {
          console.log(`🔥 Fetching top losers... Sorting: ${sortBy} (${sortOrder})`);
          ...
          if (!data?.top_losers || !Array.isArray(data.top_losers) || data.top_losers.length === 0) {
            console.warn("❌ No top losers found.");
            setTrendingCoinsData([]);
            setIsDataFetched(false);
            return;
          }
    
          let sortedData = data.top_losers.map((coin) => ({
            id: coin.id,
            name: coin.name,
            symbol: coin.symbol.toUpperCase(),
            price: coin.usd, // ✅ Fix: Use `usd`
            price_change_percentage_24h: coin.usd_24h_change ?? 0, // ✅ Fix: Use `usd_24h_change`
            image: coin.image || "",
            CoinChartColor: coin.usd_24h_change > 0 ? "#00CE39" : "#FF1B1E",
          }));
    
          console.log("📊 Sample Coin Data:", sortedData[0]); // ✅ Debugging
  ...
  }, [SearchIndex3, fetchLosersCoins]); // ✅ Depend on `fetchLosersCoins`
...
<ScrollView horizontal style={{
 ...
 <TouchableOpacity onPress={() => {
  ...
  fetchLosersCoins(newSortOrder, "price");
  ...
  fetchLosersCoins(newSortOrder, "24h");
        }}
        style={{
             backgroundColor:  CurrentViewMode.Mode_Secbg_Buttons_Cash,
             paddingVertical:  height(1) ,
