...
export default function SearchFilter_PlayToEarn({ SearchIndex3 }) {
  ...
  const [p2eCoins, setP2eCoins] = useState([]);
  ...
  // ✅ Memoize data
  const p2eCoinsMemoized = useMemo(() => p2eCoins, [p2eCoins]);
 ...
 // ✅ Fetch Play-to-Earn Coins (From Correct Category)
 const fetchP2ECoins = useCallback(
  debounce(async (sortOrder, sortBy) => {
    if (isFetching) return;
    setIsFetching(true);

    try {
      console.log(`🔥 Fetching Play-to-Earn Coins... Sorting: ${sortBy} (${sortOrder})`);

      const response = await fetch(
        "https://pro-api.coingecko.com/api/v3/coins/markets?vs_currency=usd&category=play-to-earn&order=market_cap_desc&per_page=100&page=1&price_change_percentage=1h,24h,7d",
        options
      );
      const data = await response.json();

      if (!Array.isArray(data) || data.length === 0) {
        console.warn("❌ No Play-to-Earn Coins found.");
        setP2eCoins([]);
      ...
      setP2eCoins([...sortedData]);
      setIsDataFetched(true);
    } catch (error) {
      console.error("🚨 Error fetching Play-to-Earn Coins:", error);
      setIsDataFetched(false);
    } finally {
      setIsFetching(false);
    }
...
}, [SearchIndex3, fetchP2ECoins]); // ✅ Depend on `fetchP2ECoins`
...
<ScrollView horizontal style={{
 ...
 <TouchableOpacity onPress={() => {
  ...
  fetchP2ECoins(newSortOrder, "price");
     }}
     style={{
        ...
        fetchP2ECoins(newSortOrder, "24h");
        }}
        style={{
             backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
             paddingVertical:  height(1) ,
        ...
        onPress={() => {
          ...
          fetchP2ECoins(newSortOrder, "volume");
      }}
        style={{
         backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
         paddingVertical:  height(1) ,
        ...
        <TouchableOpacity 
          onPress={() => {
            ...
            fetchP2ECoins(newSortOrder, "market_cap");
        }}
        style={{
         backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
         paddingVertical:  height(1) ,
