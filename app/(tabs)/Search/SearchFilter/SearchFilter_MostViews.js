...
export default function SearchFilter_MostViewed({ SearchIndex3 }) {
  ...
  const [mostViewedCoins, setMostViewedCoins] = useState([]);
  ...
  const mostViewedCoinsMemoized = useMemo(() => [...mostViewedCoins], [mostViewedCoins]);
  ...
  const options = {
   ...
   // ✅ Fetch Most Viewed Coins (Trending Coins API, assumes last 24h)
   const fetchMostViewedCoins = useCallback(
    debounce(async (sortOrder, sortBy) => {
      if (isFetching) return;
      setIsFetching(true);
  
      try {
        console.log(`🔥 Fetching most viewed coins... Sorting: ${sortBy} (${sortOrder})`);
        ...
        if (!data?.coins || !Array.isArray(data.coins) || data.coins.length === 0) {
          console.warn("❌ No most viewed coins found.");
          setMostViewedCoins([]);
          setIsDataFetched(false);
          return;
        }
  
        let sortedData = data.coins.map((coin) => ({
          id: coin.item.id,
          name: coin.item.name,
          symbol: coin.item.symbol.toUpperCase(),
          price: coin.item.data?.price ?? 0,
          price_change_percentage_1h: coin.item.data?.price_change_percentage_1h ?? 0,
          price_change_percentage_24h: coin.item.data?.price_change_percentage_24h ?? 0,
          price_change_percentage_7d: coin.item.data?.price_change_percentage_7d ?? 0,
          volume: coin.item.data?.total_volume ?? 0,
          market_cap: coin.item.data?.market_cap ?? 0,
          image: coin.item.large || coin.item.thumb || coin.item.small || "",
        }));
        ...
        switch (sortBy) {
          case "price":
            ...
            break;
          case "1h":
            sortedData.sort((a, b) => (sortOrder === "percent_change_1h_desc" ? b.price_change_percentage_1h - a.price_change_percentage_1h : a.price_change_percentage_1h - b.price_change_percentage_1h));
            break;
          case "24h":
            ...
            break;
          case "7d":
            sortedData.sort((a, b) => (sortOrder === "percent_change_7d_desc" ? b.price_change_percentage_7d - a.price_change_percentage_7d : a.price_change_percentage_7d - b.price_change_percentage_7d));
            break;
          case "volume":
            sortedData.sort((a, b) => (sortOrder === "volume_desc" ? b.volume - a.volume : a.volume - b.volume));
            break;
          case "market_cap":
        ...
        setMostViewedCoins([...sortedData]);
        setIsDataFetched(true);
      } catch (error) {
        console.error("🚨 Error fetching most viewed coins:", error);
        setIsDataFetched(false);
      } finally {
        setIsFetching(false);
      }
  ...
  useEffect(() => {
    ...
    if (SearchIndex3 === 0) {
      fetchMostViewedCoins();
    } else if (SearchIndex3 === -1) {
      setMostViewedCoins([]);
      setIsDataFetched(false);
    }
  }, [SearchIndex3, fetchMostViewedCoins]); // ✅ Depend on `fetchMostViewedCoins`
...
<ScrollView horizontal style={{
 ...
 <TouchableOpacity onPress={() => {
  ...
  fetchMostViewedCoins(newSortOrder, "price")
