...
export default function SearchFilter_Meme({ SearchIndex3 }) {
  ...
  // ✅ Memoize data
  const memeCoinsMemoized = useMemo(() => memeCoins, [memeCoins]);
  ...
  const options = {
   ...
   // ✅ Fetch Meme Coins (From Meme Category)
   const fetchMemeCoins = useCallback(
    debounce(async (sortOrder, sortBy) => {
      ...
      try {
        console.log(`🔥 Fetching Meme Coins... Sorting: ${sortBy} (${sortOrder})`);
  
        const response = await fetch(
          "https://pro-api.coingecko.com/api/v3/coins/markets?vs_currency=usd&category=meme-token&order=market_cap_desc&per_page=100&page=1&price_change_percentage=1h,24h,7d",
          options
        );
        const data = await response.json();
  
        if (!Array.isArray(data) || data.length === 0) {
          console.warn("❌ No Meme Coins found.");
          setMemeCoins([]);
  ...
  }, [SearchIndex3, fetchMemeCoins]); // ✅ Depend on `fetchMemeCoins`
...
<ScrollView horizontal style={{
 ...
 <TouchableOpacity onPress={() => {
  ...
  fetchMemeCoins(newSortOrder, "price");
  ...
  fetchMemeCoins(newSortOrder, "1h");
        }}
        style={{
             backgroundColor:CurrentViewMode.Mode_Secbg_Buttons_Cash,
             paddingVertical:  height(1) ,
  ...
  fetchMemeCoins(newSortOrder, "24h");
        }}
        style={{
             backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
             paddingVertical:  height(1) ,
  ...
  fetchMemeCoins(newSortOrder, "7d");
        }}
        style={{
             backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
             paddingVertical:  height(1) ,
        ...
        onPress={() => {
          ...
          fetchMemeCoins(newSortOrder, "volume");
      }}
        style={{
         backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
         paddingVertical:  height(1) ,
        ...
        <TouchableOpacity 
          onPress={() => {
            ...
            fetchMemeCoins(newSortOrder, "market_cap");
        }}
        style={{
         backgroundColor: CurrentViewMode.Mode_Secbg_Buttons_Cash,
         paddingVertical:  height(1) ,
