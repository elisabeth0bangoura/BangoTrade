...
useEffect(() => {
  posthog.capture('screen_viewed', {
    screen: 'CryptoDonutChart',
    $screen_name: 'CryptoDonutChart',
  ...
  // ✅ Updated dominant color function with `.then()` and cache
  const colorCache = {};
  ...
  useEffect(() => {
    const fetchAlpacaPositions = async () => {
      try {
        ...
        const cryptoAssets = data.filter(asset => asset.asset_class === 'crypto');
  
        let totalValue = 0;
        const pieDataArray = [];
  
        // Calculate total portfolio value (crypto only)
        ...
        const promises = cryptoAssets.map(async (asset) => {
          ...
          if (searchData.coins && searchData.coins.length > 0) {
            ...
            if (coinGeckoData && coinGeckoData.length > 0) {
              ...
              const coinValueUSD = currentCoin.current_price;
              const assetValueUSD = qty * coinValueUSD;
              ...
              if (FilterState === "Since purchase in %") {
                const percentageChangeSincePurchase = ((current_price - avg_entry_price) / avg_entry_price) * 100;
                FilteredValue = percentageChangeSincePurchase.toFixed(2);
              } else if (FilterState === "Since purchase in $") {
                const valueChangeSincePurchase = (current_price - avg_entry_price) * qty;
                FilteredValue = valueChangeSincePurchase.toFixed(2);
              } else {
                FilteredValue = 0.00;
              }
  
              const color = await fetchDominantColor(currentCoin.image);
        ...
        console.log('Final pie data:', pieDataArray);
      } catch (err) {
        setIsLoading(false);
...
{/*
      <View
          ...
          {renderDot('#FF8C03')}
          <Text style={{color: 'white'}}>Bitcoin: 47%</Text>
        </View>
        <View
          style={{flexDirection: 'row', alignItems: 'center', width: 120}}>
          {renderDot('#FF007A')}
          <Text style={{color: 'white'}}>Uniswap: 16%</Text>
        </View>*/}
