...
const extractRGBFromBase64 = (base64) => {
  ...
  return {
    ...
    b: binary.charCodeAt(2),
      ...
      useEffect(() => {
        const fetchAlpacaOptions = async () => {
          try {
            ...
            // ✅ Detect Options using Alpaca's `contract_type`
            const optionsOnly = data.filter(
              (asset) => asset.contract_type === "call" || asset.contract_type === "put"
            );
      
            const pieDataArray = [];
            let totalValue = 0;
      
            // ✅ Step 1: Get total value of all options
            optionsOnly.forEach((asset) => {
              const assetValue = asset.qty * asset.current_price;
              totalValue += assetValue;
            });
      
            const promises = optionsOnly.map(async (asset) => {
              ...
              pieDataArray.push({
                ...
                image: null, // Options usually don’t have logos
                color: CurrentViewMode.Mode_fontColor,
                FilteredValue,
              });
            });
            ...
            console.log("✅ Final pie data (Options):", pieDataArray);
          } catch (err) {
            console.error("❌ Error fetching Alpaca Options data:", err);
            setIsLoading(false);
          }
        };
