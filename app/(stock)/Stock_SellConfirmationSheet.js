...
import SellConfirmationSheet from '../(coin)/SellConfirmationSheet';
...
export const StockSellOrderTypeSheetPage = () => {
  ...
  const Stock_Sell_OrderType_Sheet = useRef(null)
  const {   
    AmountIndex, 
      setAmountIndex, 
      value, 
    ...
    useEffect(() => {
      posthog.capture('screen_viewed', {
        screen: 'Stock_Sell_OrderType_Sheet',
        $screen_name: 'Stock_Sell_OrderType_Sheet '+" / "+coinData.name,
  ...
  return(
    ...
    <ActionSheet  ref={Stock_Sell_OrderType_Sheet}
       ...
       style={{
         ...
         <ScrollView style={{
           ...
           <TouchableOpacity onPress={() => {
            ...
            posthog.capture('open_stock_sellamounts_bottomsheet', {
              screen: 'Stock_Sell_OrderType_Sheet',
              $screen_name: 'Stock_Sell_OrderType_Sheet '+" / "+coinData.name,
              timestamp: new Date().toISOString(),
              });
           ...
           //  SheetManager.hide("OrderType_Sheet");
             SheetManager.show("Stock_SellAmounts_Sheet");
           ...
           <TouchableOpacity onPress={() => {
            ...
            posthog.capture('open_stock_sellamounts_shares_bottomsheet', {
              screen: 'Stock_Sell_OrderType_Sheet',
              $screen_name: 'Stock_Sell_OrderType_Sheet '+" / "+coinData.name,
              timestamp: new Date().toISOString(),
              });
...
const StockSellConfirmationSheet =  React.memo(() => {
  const posthog = usePostHog(); // ✅ this gives you access to the actual instance
  ...
  const Stock_SellConfirmation_Sheet = useRef(null); // ✅ Correct ref type
  ...
  const formatQuantity = (value) => {
    ...
    useEffect(() => {
      ...
      const calculateFeesAndProceeds = async () => {
        ...
        // ✅ Step 1: Convert USD amount to `SellQty`
        const SellQty = rawValue / pricePerUnit; // Convert $ to crypto amount
    
        if (SellQty > totalQty) {
          console.warn("⚠️ User is trying to sell more than available balance.");
          return;
        }
    
        // ✅ Step 2: Get USD value after conversion
        const sellUsdValue = SellQty * pricePerUnit;
    
        // ✅ Step 3: Fetch trading volume and determine fee tier
        ...
        if (isMounted) {
          ...
          setFees(takerFeeInUSD.toFixed(6)); // 🔥 **Taker Fee**
        ...
        console.log(`💰 Selling: ${SellQty.toFixed(6)} BTC`);
...
return(
 ...
 <ActionSheet  id="Stock_SellConfirmation_Sheet"
  ref={Stock_SellConfirmation_Sheet}
  ...
  onClose={() => {
    posthog.capture('closed_sheet', {
      screen: 'Stock_SellConfirmation_Sheet',
      $screen_name: 'Stock_SellConfirmation_Sheet '+" / "+coinData.name,
      timestamp: new Date().toISOString(),
      });
...
{t("SellButtonTitleSellConfirmationSheet")} {formatQuantity(sellQty)} {t("OfYourPositionTextSellConfirmationSheet")} 
    </Text>
     ...
     {t("SmallTextAlpacaSellConfirmationSheet1")} <Text style={{color: "#691AF5",  fontWeight: "bold", fontSize: size(13),}}>{t("SmallTextAlpacaSellConfirmationSheet2")} </Text> {t("SmallTextAlpacaSellConfirmationSheet3")}
        ...
        posthog.capture('canceled_checkout_selling_asset', {
          ...
          timestamp: new Date().toISOString(),
           ...
           SheetManager.hide("Stock_SellConfirmation_Sheet")}}
        style={{
            height: size(55),
            width: size(55),
            marginLeft: width(5),
...
export default StockSellConfirmationSheet;