...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const calculateCost = async () => {
      ...
      console.log(`🔢 Quantity (Fixed for BTC): ${quantity}`);
    };
...
const BuyConfirmation_Sheet = useRef(null);
...
const BuyCrypto = async () => {
  const options = {
    ...
    body: JSON.stringify({
      ...
      notional: parseFloat(rawValue).toFixed(2),
    //qty: sellQty,
  })
};
...
export default BuyConfirmationSheet;