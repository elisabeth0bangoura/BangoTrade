...
const PriceTraackersheet =  React.memo(({ sheetId, payload }) => {
  ...
  const { coinData, dominantColor } = payload || {}; // Extract coinData properly
    ...
    const {
      ...
      setCurrentBackgroundColorForCoin  } = useContext(CoinPageContext);
...
const animatedStyleShares = {
  ...
  const PriceTracker_Sheet = useRef(null);
...
return(
 ...
 <ActionSheet  id={sheetId}
  ref={PriceTracker_Sheet}
  backgroundInteractionEnabled={false}
  gestureEnabled={true}
  isModal={false} 
  onClose={() => {
    toastTriggered.current = false;
    setShowPriceTickerSaved(false);
    setPlacedTracker(false); // ✅ Reset only now
...
setPlacedTracker(true)
        ...
        const SavePriceTracker = async () => {
          try {
            ...
            await Promise.all([
              SheetManager.hide("PriceTraackersheetRef"),
...
export default PriceTraackersheet;